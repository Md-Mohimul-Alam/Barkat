...
const LoadingPointForm = () => {
  ...
  const fields = [
    ...
    { label: 'Type', name: 'type', type: 'select', required: true, options: ['Port', 'Storage', 'Vessel', 'Local', 'Factory', 'Warehouse'] },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await loadingPointService.createLoadingPoint(formData);
      if (response.success) {
        alert('Loading point created successfully!');
        navigate('/app/loading-points');
      }
    } catch (error) {
      console.error('Error creating loading point:', error);
      alert(error.message || 'Failed to create loading point');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex-1 p-6 overflow-auto">
          <div className="max-w-4xl mx-auto">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-2xl font-bold">Add Loading Point</h1>
              <button
                onClick={() => navigate('/app/loading-points')}
            ...
            <div className={`rounded-xl p-8 transition-all duration-300 ${
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2 flex justify-center gap-4 pt-4">
                  ...
                  >
                    {loading ? 'Creating...' : 'Create Loading Point'}
                  </button>
                </div>
              </form>
            </div>
